...
<div class="space-y-12">
    ...
    <div class="bg-slate-900 rounded-2xl sm:rounded-[3rem] p-5 sm:p-8 md:p-12 text-white relative overflow-hidden shadow-2xl shadow-indigo-100">
        ...
        <div class="relative z-10 space-y-3 sm:space-y-4">
            <h5 class="text-lg sm:text-xl md:text-2xl font-bold flex items-center justify-center sm:justify-start gap-2 sm:gap-3">
                <i data-lucide="terminal" class="text-indigo-400 w-5 h-5 sm:w-6 sm:h-6"></i> Implementasi Antrean
            </h5>
    ...
    <section class="space-y-4 sm:space-y-6">
        <div class="flex items-center gap-3 sm:gap-4">
            ...
            <h3 class="text-lg sm:text-xl md:text-2xl font-bold text-slate-800">Enqueue (Tambah ke Belakang)</h3>
        </div>
        <div class="ml-0 space-y-3 sm:space-y-4">
            <p class="text-slate-600 text-xs sm:text-sm">
                Sama seperti List, kita gunakan <code class="bg-slate-100 px-2 py-1 rounded text-indigo-600 font-bold text-xs sm:text-sm">append()</code>. Fungsi ini otomatis meletakkan data di urutan paling akhir (Rear).
            </p>
            <div >
    ...
    <section class="space-y-4 sm:space-y-6">
        <div class="flex items-center gap-3 sm:gap-4">
            ...
            <h3 class="text-lg sm:text-xl md:text-2xl font-bold text-slate-800">Front (Melihat Data Terdepan)</h3>
        </div>
        <div class="ml-0 space-y-3 sm:space-y-4">
            <p class="text-slate-600 text-xs sm:text-sm">
                Karena antrean dimulai dari depan, maka data yang akan dilayani selalu berada pada <code class="bg-slate-100 px-2 py-1 rounded text-indigo-600 font-bold text-xs sm:text-sm">index [0]</code>.
            </p>
            <div >
                <div class="overflow-x-auto" style="max-height: 400px;">
                    <div class="bg-slate-900 rounded-lg p-4 sm:p-6 font-mono text-xs sm:text-sm leading-relaxed shadow-xl border border-slate-700 inline-block min-w-full">
    ...
    <section class="space-y-4 sm:space-y-6">
        <div class="flex items-center gap-3 sm:gap-4">
            ...
            <h3 class="text-lg sm:text-xl md:text-2xl font-bold text-slate-800">Dequeue (Hapus dari Depan)</h3>
        </div>
        <div class="ml-0 space-y-3 sm:space-y-4">
            <p class="text-slate-600 text-xs sm:text-sm">
                Gunakan fungsi <code class="bg-slate-100 px-2 py-1 rounded text-rose-600 font-bold text-xs sm:text-sm">pop(0)</code>. Angka 0 wajib diisi agar Python menghapus elemen terdepan, bukan terakhir.
            </p>
    ...
    <section class="space-y-4 sm:space-y-6">
        <div class="flex items-center gap-3 sm:gap-4">
            ...
            <h3 class="text-lg sm:text-xl md:text-2xl font-bold text-slate-800">Pengecekan & Perulangan</h3>
        </div>
        <div class="ml-0 space-y-3 sm:space-y-4">
            <p class="text-slate-600 text-xs sm:text-sm">
                Kita bisa mengecek panjang antrean dengan <code class="bg-slate-100 px-2 py-1 rounded text-indigo-600 font-bold text-xs sm:text-sm">len()</code> dan menampilkannya dengan perulangan.
            </p>
            <div >
                <div class="overflow-x-auto" style="max-height: 400px;">
                    <div class="bg-slate-900 rounded-lg p-4 sm:p-6 font-mono text-xs sm:text-sm leading-relaxed shadow-xl border border-slate-700 inline-block min-w-full">
    ...
    <div class="mt-20 pt-10 border-t border-slate-100 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
        <a href="{{ url('/materi/Simulasi-queue') }}" class="group w-full sm:w-auto text-slate-400 hover:text-indigo-600 font-bold flex items-center justify-center sm:justify-start gap-2 transition text-sm sm:text-base order-2 sm:order-1 text-center sm:text-left">
            <i data-lucide="arrow-left" class="w-4 h-4 sm:w-5 sm:h-5 group-hover:-translate-x-1 transition-transform hidden sm:block"></i> Kembali ke Simulasi Queue
        </a>
        <a href="{{ url('/materi/materi-stack') }}" class="group flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-700 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base transition-all shadow-lg shadow-indigo-200 order-1 sm:order-2 w-full sm:w-auto">
